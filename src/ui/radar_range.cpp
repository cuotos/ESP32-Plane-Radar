...
constexpr char kPrefsFlightLevelsKey[] = "altFL";
...
void saveFlightLevels() {
  if (!s_prefs.begin(kPrefsNamespace, false)) {
    return;
  }
  s_prefs.putBool(kPrefsFlightLevelsKey, s_flight_levels);
  s_prefs.end();
}
...
void rangeInit() {
  ...
  s_flight_levels = s_prefs.getBool(kPrefsFlightLevelsKey, true);
  s_prefs.end();
}
...
bool flightLevels() { return s_flight_levels; }
...
void saveFlightLevelsFromPortal(const char* checkbox_value) {
  s_flight_levels = portalCheckboxChecked(checkbox_value);
  saveFlightLevels();
...
void unitsReset() {
  s_use_miles = false;
  s_show_runways = true;
  s_flight_levels = true;
  if (s_prefs.begin(kPrefsNamespace, false)) {
    s_prefs.remove(kPrefsMilesKey);
    s_prefs.remove(kPrefsRunwaysKey);
    s_prefs.remove(kPrefsFlightLevelsKey);
    s_prefs.end();
  }
}
